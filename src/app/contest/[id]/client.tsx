          ...
          <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-8">
            ...
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-800/50 border border-slate-700/50 rounded-sm mb-4">
                <span
                  className={`w-2 h-2 rounded-full ${contest.status === 'Active' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-500'}`}
                ></span>
          ...
          <div>
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="group p-4 bg-surface border border-slate-800 hover:border-slate-600 transition-colors rounded-sm cursor-pointer"
                >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {['PyTorch', 'TensorFlow', 'Python', 'Docker'].map((tag) => (
                <span
                  key={tag}
                  className="text-xs text-slate-400 bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full"
                >
                  {tag}
                </span>
              ))}
