  ...
  const filteredContests = useMemo(() => {
    return initialContests.filter((c) => {
      ...
      if (
        ...
        !filters.platforms.includes(c.platform)
      )
        return false;
          ...
          <div className="flex-1">
              ...
              {filteredContests.length > 0 ? (
                filteredContests.map((contest) => (
                  <ContestCard
                    ...
                    onToggleSubscription={(status) =>
                      handleToggleSubscription(contest.id, status)
                    }
                  />
                ))
              ) : (
                <div className="py-20 text-center border border-dashed border-slate-800 rounded-sm">
                  ...
                  <p className="text-slate-600">
                    Try adjusting your sensors (filters).
                  </p>
                </div>
              )}
            </div>
            ...
            {filteredContests.length > 5 && (
              <div className="mt-8 flex justify-end gap-2 text-sm font-mono">
                <button className="px-3 py-1 bg-slate-900 border border-slate-800 text-slate-500 hover:border-primary hover:text-primary">
                  &lt;
                </button>
                <button className="px-3 py-1 bg-primary text-white font-bold">
                  1
                </button>
                <button className="px-3 py-1 bg-slate-900 border border-slate-800 text-slate-400 hover:border-primary hover:text-primary">
                  2
                </button>
                <button className="px-3 py-1 bg-slate-900 border border-slate-800 text-slate-400 hover:border-primary hover:text-primary">
                  3
