...
export function ContestCard({ contest, isSubscribed = false, onToggleSubscription = () => {} }: ContestCardProps) {
  ...
  return (
    <div className="flex flex-col md:flex-row bg-surface border border-slate-800 rounded-sm hover:border-secondary transition-all group overflow-hidden">
        ...
        <div className="w-full md:w-48 bg-slate-900/30 p-4 md:p-5 border-t md:border-t-0 md:border-l border-slate-800 flex flex-row md:flex-col justify-between items-center md:items-end">
            ...
            <Link href={`/contest/${contest.id}`} className="flex items-center gap-2 text-xs font-bold text-primary hover:text-white transition-colors uppercase border border-primary/20 hover:bg-primary/20 px-4 py-2 rounded-sm md:w-full md:justify-center mt-2">
