  ...
  return (
    <aside className="w-full lg:w-64 flex-shrink-0 space-y-8 sticky top-24 self-start">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() =>
            setFilters({ platforms: [], types: [], status: 'active' })
          }
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {['ModelScope', 'Civitai', 'Kaggle', 'DrivenData', 'AIcrowd'].map(
            (p) => (
              <label
                key={p}
              ...
              >
                <div
                  className={`w-4 h-4 border rounded-sm flex items-center justify-center transition-all ${
                    filters.platforms.includes(p)
                      ...
                      : 'border-slate-600 bg-canvas group-hover:border-primary'
                  }`}
                >
                  {filters.platforms.includes(p) && (
                ...
                />
                <span
                  className={`text-sm ${filters.platforms.includes(p) ? 'text-white' : 'text-slate-400 group-hover:text-slate-200'}`}
                >
                  {p}
                </span>
              </label>
