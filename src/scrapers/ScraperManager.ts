...
export class ScraperManager {
  ...
  private createScraper(config: PlatformConfig): BaseScraper | null {
    switch (config.name.toLowerCase()) {
      ...
      default:
        logger.warn(`No scraper implementation found for platform: ${config.name}`);
        return null;
    }
  }
  ...
  async scrapeAll(maxConcurrency = 3): Promise<Map<string, RawContest[]>> {
    ...
    logger.info(`Starting scrape for ${scrapers.length} platforms with concurrency: ${maxConcurrency}`);
    ...
    for (let i = 0; i < scrapers.length; i += maxConcurrency) {
      ...
      batchResults.forEach((result, index) => {
        ...
        if (result.status === 'fulfilled') {
          ...
          logger.info(`Successfully scraped ${result.value.length} contests from ${platformName}`);
        } else {
          logger.error(`Failed to scrape ${platformName}`, { error: result.reason });
          results.set(platformName, []);
        }
      });
    }

    const totalContests = Array.from(results.values()).reduce((sum, contests) => sum + contests.length, 0);
   ...
   */
  async scrapeSpecific(platformNames: string[]): Promise<Map<string, RawContest[]>> {
    const results = new Map<string, RawContest[]>();
   ...
   */
  private async scrapeWithRetry(platformName: string, scraper: BaseScraper): Promise<RawContest[]> {
    return retry(
      async () => {
        logger.info(`Starting scrape for ${platformName}`);
        const contests = await scraper.scrape();
        logger.info(`Completed scrape for ${platformName}: ${contests.length} contests`);
        return contests;
      },
      3, // max attempts
      2000 // base delay
  ...
  getScrapingStats(): Record<string, any> {
    const stats = {
      ...
      enabledPlatforms: this.sourceManager ? this.sourceManager.listPlatforms(true).length : 0,
