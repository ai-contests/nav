...
import { ProcessedContest, RawContest, StorageConfig, StorageResult } from '../types';
...
export class StorageManager {
  ...
  private async ensureDirectories(): Promise<void> {
    try {
      ...
      logger.info('Storage directories initialized', { dataDir: this.config.dataDir });
    } catch (error) {
   ...
   */
  async saveRawContests(contests: RawContest[], platform: string): Promise<StorageResult> {
    try {
      ...
      const latestPath = path.join(this.config.dataDir, 'raw', `${platform}-latest.json`);
      if (await fs.pathExists(latestPath)) {
        await fs.remove(latestPath);
      }
      await fs.copy(filePath, latestPath);

      logger.info(`Saved ${contests.length} raw contests for ${platform}`, { filePath });
   ...
   */
  async saveProcessedContests(contests: ProcessedContest[], platform?: string): Promise<StorageResult> {
    try {
      const timestamp = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
      const filename = platform ? `${platform}-${timestamp}.json` : `all-contests-${timestamp}.json`;
      ...
      // Update latest symlink
      const latestFilename = platform ? `${platform}-latest.json` : 'all-contests-latest.json';
      const latestPath = path.join(this.config.dataDir, 'processed', latestFilename);
      if (await fs.pathExists(latestPath)) {
        await fs.remove(latestPath);
      }
      await fs.copy(filePath, latestPath);

      logger.info(`Saved ${contests.length} processed contests`, { filePath, platform });
   ...
   */
  async loadRawContests(platform?: string, date?: string): Promise<RawContest[]> {
    try {
      let filePath: string;

      if (platform && date) {
        filePath = path.join(this.config.dataDir, 'raw', `${platform}-${date}.json`);
      } else if (platform) {
        filePath = path.join(this.config.dataDir, 'raw', `${platform}-latest.json`);
      } else {
   ...
   */
  async loadProcessedContests(platform?: string, date?: string): Promise<ProcessedContest[]> {
    try {
      let filePath: string;

      if (platform && date) {
        filePath = path.join(this.config.dataDir, 'processed', `${platform}-${date}.json`);
      } else if (platform) {
        filePath = path.join(this.config.dataDir, 'processed', `${platform}-latest.json`);
      } else {
        filePath = path.join(this.config.dataDir, 'processed', 'all-contests-latest.json');
      }
      ...
      logger.error('Failed to load processed contests', { error, platform, date });
      return [];
    }
  }
  ...
  private async cleanOldBackups(): Promise<void> {
    try {
      ...
      if (files.length > this.config.backupCount) {
        ...
        const filesToDelete = fileStats.slice(0, files.length - this.config.backupCount);
  ...
  async exportData(format: 'json' | 'csv', platform?: string): Promise<string> {
    ...
    if (format === 'json') {
      ...
      // Simple CSV export
      const csvHeader = 'Title,Platform,URL,Status,Deadline,Prize,Description\n';
      const csvRows = contests.map(contest => 
        [
    ...
    logger.info(`Exported ${contests.length} contests to ${format.toUpperCase()}`, { filePath });
    return filePath;
  }
