...
async function testPipeline() {
  ...
  try {
    ...
    if (rawContests.length === 0) {
      console.log('ℹ️  No existing data found. You can add test data to data/raw/ directory');
      return;
    }
    
    // Test 2: Validate data
    console.log('\n🔍 Testing data validation...');
    const validationResult = await dataValidator.validateRawContests(rawContests);
    console.log(`Validation: ${validationResult.isValid ? 'PASS' : 'FAIL'}`);
    console.log(`Valid: ${validationResult.summary.validContests}, Invalid: ${validationResult.summary.invalidContests}`);
    ...
    const saveResult = await storageManager.saveProcessedContests(processedContests);
